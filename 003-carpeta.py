...
# Output file with epoch timestamp in the filename
output_file = os.path.join(render_folder, f"simulation_video_{epoch_time}.mp4")
...
# Create instances
personas = [Persona((random.uniform(-0.5, 0.5) / 40)) for _ in range(1000)]
comidas = [Comida() for _ in range(550)]
hogares = [Hogar() for _ in range(550)]
...
for frame_number in range(total_frames):
    ...
    # Every 60 frames (1 second), print statistics
    if frame_number % fps == 0:
        elapsed_time = time.time() - start_time
        remaining_time = (total_frames - frame_number) / fps
        completion_percentage = (frame_number / total_frames) * 100
        
        # Print statistics to the console
        print(f"Frame: {frame_number}, Time Passed: {elapsed_time:.2f}s, "
